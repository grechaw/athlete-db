...
(:
 ...
 The source entity type document was AthleteDb-0.0.1
...
) as map:map
{
    ...
    (:
    The following code populates the properties of the 'Player'
...
) as map:map
{
    ...
    $instance
    =>   map:with('name',                   xs:string($source-node/name))
    =>   map:with('id',                     xs:integer($source-node/id))
    =>es:optional('crest',                  sem:iri($source-node/crest))
    =>   map:with('value',                  xs:long($source-node/value))
};
